...
import { jobs } from '../../api/mock/data';
              ...
              <CardContent>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                  <Box sx={{ width: 40, height: 40, borderRadius: 1.5, bgcolor: 'primary.main', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Typography sx={{ color: '#fff', fontWeight: 700, fontSize: '0.875rem' }}>{job.company.charAt(0)}</Typography>
                  </Box>
                  <Button size="small" sx={{ minWidth: 0, p: 0.5 }} onClick={() => setSaved((p) => p.includes(job.id) ? p.filter((s) => s !== job.id) : [...p, job.id])}>
                    {saved.includes(job.id) ? <BookmarkIcon sx={{ color: 'primary.main' }} /> : <BookmarkBorderIcon sx={{ color: 'text.secondary' }} />}
                  </Button>
                </Box>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Typography variant="caption" sx={{ color: 'text.secondary' }}>{formatRelativeTime(job.posted + 'T12:00:00Z')}</Typography>
                </Box>
              </CardContent>
            </Card>
          </Grid>
