import {
  Box, Card, CardContent, Grid, LinearProgress, Typography,
} from '@mui/material';
import AddIcon from '@mui/icons-material/Add';
import PageHeader from '../../components/common/PageHeader';
import StatusBadge from '../../components/common/StatusBadge';
import { campaigns } from '../../api/mock/data';
  ...
  return (
    <Box>
      <PageHeader
        ...
        primaryAction={{ label: 'New Campaign', onClick: () => {}, icon: <AddIcon /> }}
      />
              ...
              <Card sx={{ height: '100%', transition: 'all 0.2s', '&:hover': { transform: 'translateY(-2px)', boxShadow: 3 } }}>
                <CardContent>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                    <Typography variant="caption" sx={{ color: 'text.secondary' }}>{c.type}</Typography>
                    <Typography variant="caption" sx={{ color: 'text.secondary' }}>· Started {formatDate(c.startDate)}</Typography>
                  </Box>
                  ...
                  <Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                      ...
                      <Typography variant="caption" sx={{ fontWeight: 600 }}>{openRate.toFixed(1)}%</Typography>
                    </Box>
                    <LinearProgress variant="determinate" value={openRate} sx={{ height: 4, borderRadius: 2, bgcolor: 'grey.100', '& .MuiLinearProgress-bar': { bgcolor: 'primary.main' } }} />
                  </Box>
                </CardContent>
              </Card>
            </Grid>
