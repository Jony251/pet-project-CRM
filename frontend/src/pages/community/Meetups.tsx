...
import { meetups } from '../../api/mock/data';
          ...
          return (
            <Grid key={meetup.id} size={{ xs: 12, sm: 6 }}>
              <Card sx={{ height: '100%', transition: 'all 0.2s', '&:hover': { transform: 'translateY(-2px)', boxShadow: 4 } }}>
                <Box sx={{ height: 100, background: `linear-gradient(135deg, ${tagColors[Number(meetup.id) % tagColors.length]}, ${tagColors[(Number(meetup.id) + 1) % tagColors.length]})`, borderRadius: '12px 12px 0 0' }} />
                <CardContent>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mb: 2 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                      ...
                      <Typography variant="body2" sx={{ color: 'text.secondary' }}>{formatDate(meetup.date)}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                      <LocationOnOutlinedIcon sx={{ fontSize: 16, color: 'text.secondary' }} />
                      <Typography variant="body2" sx={{ color: 'text.secondary' }}>{meetup.location}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                      <PeopleOutlinedIcon sx={{ fontSize: 16, color: 'text.secondary' }} />
                      <Typography variant="body2" sx={{ color: 'text.secondary' }}>{meetup.attendees} / {meetup.maxAttendees} attendees</Typography>
                    </Box>
                  </Box>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 2 }}>
                    {meetup.tags.map((tag) => <Chip key={tag} label={tag} size="small" variant="outlined" />)}
                  </Box>
